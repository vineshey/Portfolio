...
export default function InternshipDiary() {
  ...
  return (
    <div id="workexperience" className="relative w-full max-w-6xl mx-auto px-4 py-12 scroll-mt-32">
      <h2 className="text-3xl font-bold text-center mb-4 dark:text-white text-black">Work Experience</h2>
      <div className="mb-8"></div>

      <div className="relative flex flex-col items-center min-h-[400px] w-full aspect-[4/2] max-w-full">
        <div className="absolute -top-6 left-1/2 -translate-x-1/2 z-30 flex space-x-[6px]">
          {Array.from({ length: 15 }).map((_, i) => (
            <div
              key={i}
              className="w-3 h-8 border-[3px] border-black dark:border-white rounded-full"
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className={`relative z-10 bg-gradient-to-br ${gradients[(currentPage + 1) % gradients.length]} rounded-xl p-8 shadow-2xl w-full h-full origin-top`}
          >
            <div className="relative z-10 text-black dark:text-black">
  {currentPage === -1 ? (
    <>
      <h2 className="text-2xl font-bold text-green-800 mb-4 drop-shadow dark:text-green-300 text-center">
        Graduate Engineering Trainee – ITI Palakkad
      </h2>
      <ul className="list-disc list-inside text-gray-900 font-medium space-y-1">
      ...
      <div className="flex justify-center">
        <button
          ...
          className="mt-6 px-6 py-1 bg-green-600 text-white rounded-lg hover:bg-green-700 transition text-base"
        >
          Open Diary
                    </button>
                  </div>
                ...
                <>
                  <h2 className="text-xl font-bold text-green-700 mb-4 drop-shadow dark:text-green-200 text-center">{pages[currentPage].title}</h2>
                  <ul className="list-disc list-inside space-y-2">
                    {pages[currentPage].points.map((point, idx) => (
                      <li key={idx} className="text-base drop-shadow text-black dark:text-black">{point}</li>
                    ))}
                  </ul>
                  ...
                  <div className="flex justify-between items-center mt-8">
                    ...
                    >
                      <img src="https://cdn-icons-png.flaticon.com/512/271/271220.png" alt="prev" className="w-24 h-24" />
                    </button>
                    <span className="text-sm text-black-600 dark:text-black-300">Page {currentPage + 1} / {pages.length}</span>
                    <button
                      onClick={() => setCurrentPage(currentPage + 1)}
                      className="text-black-600 hover:brightness-110 disabled:opacity-30"
                      disabled={currentPage === pages.length - 1}
                    >
                      <img src="https://cdn-icons-png.flaticon.com/512/271/271228.png" alt="next" className="w-24 h-24" />
                    </button>
                  </div>
                  ...
                  <div className="flex justify-center">
                    <button
                      ...
                      className="mt-6 text-sm text-gray-500 dark:text-black-300 hover:text-gray-700 dark:hover:text-white"
                    >
                      ⬅ Back to Summary
                    </button>
                  </div>
